...
with lib; let
  ...
  generateDockerCompose = name: instanceCfg: config:
    let
      port = if instanceCfg.port > 0 then instanceCfg.port else (8000 + (stringLength name));
      ...
      enabledServices = lib.filterAttrs (name: service: 
        if name == "kong" then instanceCfg.services.restApi
        else if name == "auth" then instanceCfg.services.auth
        else if name == "rest" then instanceCfg.services.restApi
        else if name == "realtime" then instanceCfg.services.realtime
        else if name == "storage" then instanceCfg.services.storage
...
in {
  ...
  generateService = name: instanceCfg: config: 
    let
