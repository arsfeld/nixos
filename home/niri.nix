...
# Declarative niri config via niri-flake with waybar, mako, swaylock, fuzzel
{
  ...
  # Check `options` (declarations), not `config` — referencing config.programs here would
  # force this module's own conditional programs.niri definition and self-reference.
  hasNiriModule = options.programs ? niri;
in {
  config = lib.mkIf niriEnabled (lib.mkMerge [
    # `programs.niri` is only declared when niri-flake's HM module is loaded (the NixOS
    # path auto-imports it; standalone HM does not). Use optionalAttrs so the definition
    # is structurally absent otherwise — mkIf alone does NOT suppress the module system's
    # "option does not exist" error for an undeclared option.
