...
func main() {
	...
	// Clean up orphaned nftables rules that aren't in our state
	mappings := stateManager.GetMappings()
	removed, err := nftManager.SyncWithState(mappings)
	if err != nil {
		log.Printf("Warning: failed to sync nftables with state: %v", err)
	} else if removed > 0 {
		log.Printf("Cleaned up %d orphaned nftables rules", removed)
	}

	// Restore mappings from state to nftables
...
func (s *NATPMPServer) handleMappingRequest(data []byte, addr *net.UDPAddr, opcode byte) {
	...
	// Check if the requested external port is already in use by another client
	inUse, existingRule, err := s.nftManager.IsExternalPortInUse(protocol, externalPort)
	if err != nil {
	...
	if inUse && existingRule != nil {
		// Port is in use - check if it's by the same client for the same internal port
		if existingRule.InternalIP != addr.IP.String() || existingRule.InternalPort != internalPort {
			// Different client or different internal port - need to find a new port
			logVerbose("Port %d already in use by %s:%d, finding alternative",
				externalPort, existingRule.InternalIP, existingRule.InternalPort)

			newPort := s.findAvailablePort(protocol, externalPort)
			if newPort == 0 {
				log.Printf("No available ports for %s:%d", addr.IP, internalPort)
				requestsTotal.WithLabelValues(protocol, "error_rsrc").Inc()
				s.sendErrorResponse(addr, opcode, RESPONSE_RSRC)
				return
			}
			externalPort = newPort
			logVerbose("Allocated alternative port %d", externalPort)
		}
	}
...
// findAvailablePort finds an available external port, starting near the preferred port
func (s *NATPMPServer) findAvailablePort(protocol string, preferred uint16) uint16 {
	// Get all current rules to check what's in use
	rules, err := s.nftManager.ListRules()
	if err != nil {
		log.Printf("Failed to list rules for port allocation: %v", err)
		return 0
	}

	// Build a set of ports in use for this protocol
	inUse := make(map[uint16]bool)
	for _, rule := range rules {
		if rule.Protocol == protocol {
			inUse[rule.ExternalPort] = true
		}
	}

	// Try ports starting from preferred, then search in allowed ranges
	// First, try ports close to the preferred port
	for offset := uint16(1); offset < 1000; offset++ {
		// Try preferred + offset
		candidate := preferred + offset
		if s.isPortAllowed(candidate) && !inUse[candidate] {
			return candidate
		}
		// Try preferred - offset (if it won't underflow)
		if preferred > offset {
			candidate = preferred - offset
			if s.isPortAllowed(candidate) && !inUse[candidate] {
				return candidate
	...
	// If no port found near preferred, search the entire allowed range
	for _, r := range s.config.AllowedPorts {
