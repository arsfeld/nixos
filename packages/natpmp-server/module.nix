...
with lib; let
  ...
  configFlags =
    [
      ...
      "--nat-table"
      cfg.nftables.natTable
      "--nat-chain"
      cfg.nftables.natChain
      "--filter-table"
      cfg.nftables.filterTable
      "--filter-chain"
      cfg.nftables.filterChain
      "--max-mappings-per-ip"
      (toString cfg.maxMappingsPerClient)
      "--default-lifetime"
      (toString cfg.defaultLifetime)
      "--max-lifetime"
      (toString cfg.maxLifetime)
      "--state-dir"
      "/var/lib/natpmp-server"
      "--cleanup-interval"
      (toString cfg.cleanupInterval)
    ]
