    ...
    def _load_state(self) -> dict:
        ...
        if self.state_file.exists():
            try:
                ...
                console.print(
                    "[red]Warning: Could not load state file, creating new one[/red]"
                )
        ...
        async with httpx.AsyncClient() as client:
            for file_path in files_to_download:
                ...
                try:
                    ...
                    console.print(
                        f"[yellow]Warning: Could not download {file_path}: {e}[/yellow]"
                    )
                    continue
    ...
    def render_docker_compose(self, name: str, ports: dict, env_vars: dict) -> str:
        ...
        if not template_path.exists():
            raise FileNotFoundError(
                f"Docker compose template not found at {template_path}. Run 'supabase-manager download' first."
            )
        ...
        for old_name, new_name in container_replacements:
            rendered = rendered.replace(
                f"container_name: {old_name}", f"container_name: {new_name}"
            )
    ...
    def get_instance_status(self, name: str) -> str:
        ...
        try:
            containers = self.docker_client.containers.list(
                all=True, filters={"label": f"supabase.instance={name}"}
            )
...
class DashboardScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Container(DataTable(id="instances_table"), id="dashboard")
        yield Footer()
...
def cli(command: Optional[str], name: Optional[str]):
    ...
    if command == "create":
        ...
        console.print(
            "Available commands: create, list, start, stop, delete, start-all, stop-all, download, maintenance"
        )
        sys.exit(1)
