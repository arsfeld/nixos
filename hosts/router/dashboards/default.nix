...
let
  ...
  createRow = { title, y, id, collapsed ? false }: {
    collapsed = collapsed;
  ...
  repositionPanels = startY: panels:
    let
      ...
      calculatePositions = panels: currentY: 
        if panels == [] then []
          ...
          let
            ...
            actualNextY = if needsNewRow then nextY else currentY;
          in
  ...
  organizePanelsInGrid = startY: panelsPerRow: panels:
    let
      ...
      arrangePanels = panels: row: col:
        if panels == [] then []
          ...
          let
            ...
            nextCol = if col + 1 >= panelsPerRow then 0 else col + 1;
            nextRow = if col + 1 >= panelsPerRow then row + 1 else row;
          in
  ...
  sectionHeight = panels:
    if panels == [] then 1
      ...
      let
        maxY = lib.foldl' (max: panel: 
          let panelBottom = (panel.gridPos.y or 0) + (panel.gridPos.h or 8);
          in if panelBottom > max then panelBottom else max
        ) 0 panels;
      in maxY;
  ...
  buildSections = 
    let
      ...
      systemRow = createRow { title = "System Overview"; y = currentY; id = 100; };
      ...
      networkRow = createRow { title = "Network Interfaces"; y = networkY; id = 101; };
      ...
      clientRow = createRow { title = "Client Traffic"; y = clientY; id = 102; };
      ...
      dnsRow = createRow { title = "DNS"; y = dnsY; id = 103; };
      ...
      qosRow = createRow { title = "QoS / Traffic Shaping"; y = qosY; id = 104; };
      ...
      upnpRow = createRow { title = "UPnP / Port Forwarding"; y = upnpY; id = 105; };
      ...
      speedRow = createRow { title = "Internet Speed Test"; y = speedY; id = 106; };
      ...
      otherRow = createRow { title = "Other Metrics"; y = otherY; id = 107; };
      otherPanelsRepositioned = if uncategorizedPanels != [] 
        then organizePanelsInGrid (otherY + 1) 3 uncategorizedPanels
        else [];
    in
      # Combine all sections
      [systemRow] ++ systemPanelsRepositioned ++
      [networkRow] ++ networkPanelsRepositioned ++
      [clientRow] ++ clientPanelsRepositioned ++
      [dnsRow] ++ dnsPanelsRepositioned ++
      [qosRow] ++ qosPanelsRepositioned ++
      [upnpRow] ++ upnpPanelsRepositioned ++
      [speedRow] ++ speedPanelsRepositioned ++
      (if uncategorizedPanels != [] then [otherRow] ++ otherPanelsRepositioned else []);
