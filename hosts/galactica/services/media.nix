...
in
  lib.mkMerge [
    (mkService "komga" {port = komgaPort;})
    (mkService "lidarr" {port = 8686;})
    (mkService "tautulli" {port = 8181;})
    (mkService "headphones" {port = headphonesPort;})
    (mkService "resilio" {port = 9000;})
